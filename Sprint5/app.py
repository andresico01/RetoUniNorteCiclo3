...
# Clase para administrar los Blogs del Usuario
class BlogBD:
    # Metodo constructor de la clase
    def __init__(self, idBlog, Usuario, titulo, textoPost, publico ):
        self.idBlog = idBlog
        self.titulo = titulo
        self.textoPost = textoPost
...
@app.route('/modificarBlog',methods=['POST'])
def modificarBlog():
    idBlog=request.form['idBlog']
    Titulo=request.form['titulo']
    Contenido=request.form['textoPost']
    ...
    conexion.execute("update T_Blogs set Correo='"+Correo+"', Titulo = '"+Titulo+"', Contenido ='"+Contenido+"', Publico ='"+Publico+"' where Blog_id ='"+idBlog+"'")
    #conexion.execute("update T_Blogs set Titulo ='POST UNO CORREGIDO', Contenido='PORCONSOLA DE SQLLITE ESTE ES ELTEXTO DELPOST UNO CORREGIDO DESPUES DE HACER ELUPDATE', Publico='NO' WHERE Blog_id = 3")
    conexion.commit()
    conexion.close()
    #mensaje=Mensaje("NOTIFIACION","Tu blog a sigo guardado exitosamente",True)
    return redirect(url_for('principal'))

    #conexion a base de datos
    #objcursor.execute("select * from T_Blogs where Blog_id = '"+idBlog+"'")
...
def modif():
    ...
    return render_template('modificar.html', idBlog=idBlog, titulo=titulo, textoPost=textoPost, publico=publico)

@app.route('/elm')
def elm():
    return redirect(url_for('elimin'))
...
@app.route('/elminar', methods=['POST'])
def elimin():
    idBlog=request.form['idBlog']
    ...
    return render_template('eliminar.html', idBlog=idBlog, titulo=titulo, textoPost=textoPost, publico=publico)

@app.route('/elminarBlog',methods=['POST'])
def eliminarBlog():
    idBlog=request.form['idBlog']
    return render_template('eliminar.html')
...
def principal():
    if 'user' in session:
        ...
        conexion.row_factory = sql.Row
        objcursor = conexion.cursor()
        objcursor.execute("select * from T_Blogs where Correo = '"+email+"'")
        rows = objcursor.fetchall()
        conexion.close()
        blogs_list = []
        ...
        for r in range(0,len(rows)):
            ...
            textoPost=rows[r][3]
            publico=rows[r][4]
            blog = BlogBD(idBlog,usuario,titulo,textoPost,publico)
            blogs_list.append(blog)
            print("listado de blogs",blogs_list.count)
            print (titulo , rows[0][1])
        
        return render_template('/RealBlog.html', BlogsBD=blogs_list)
    else:
...
@app.route('/NuevoBlog', methods=['POST'])
def crearBlog():
    Titulo=request.form['Titulo']
    Contenido=request.form['textoPost']
    Publico=request.form['publico']
    ...
    #conexion con base de datos y registro de usuario
    conexion = sqlite3.connect("dbUsuarios.db")
    conexion.execute("insert into T_Blogs(Correo, Titulo, Contenido, Publico) values (?,?,?,?)" ,(Correo,Titulo,Contenido,Publico))
    conexion.commit()
    conexion.close()
    #mensaje=Mensaje("NOTIFIACION","Tu blog a sigo guardado exitosamente",True)
    return redirect(url_for('principal'))
